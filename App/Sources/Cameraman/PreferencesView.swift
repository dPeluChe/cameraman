...
private struct HotkeysPreferencesView: View {
...
/// Row showing a registered hotkey
private struct HotkeyRow: View {
    ...
    private var keyEquivalent: String {
        ...
        // Add modifiers
        if hotkey.modifiers & HotkeyManager.Hotkey.cmdKey != 0 {
            result += "⌘"
        }
        if hotkey.modifiers & HotkeyManager.Hotkey.optionKey != 0 {
            result += "⌥"
        }
        if hotkey.modifiers & HotkeyManager.Hotkey.controlKey != 0 {
            result += "⌃"
        }
        if hotkey.modifiers & HotkeyManager.Hotkey.shiftKey != 0 {
            result += "⇧"
        }
...
/// Row showing default hotkey info
private struct DefaultHotkeyRow: View {
...
private struct ExportPreferencesView: View {
...
@MainActor
private class HotkeysPreferencesViewModel: ObservableObject {
